...
def euclidean_halton_graph(n, radius, bases, lower, upper, source, target, mapFile, car_width, car_length, collision_delta):
...
def generate_graph_file(map_msg, halton_points, disc_radius, car_width, car_length, collision_delta):
  ...
  if not os.path.exists(file_name):
    ...
    upper = numpy.array([map_msg.info.origin.position.x+map_msg.info.resolution*map_msg.info.width, map_msg.info.origin.position.y+map_msg.info.resolution*map_msg.info.height])
...
if __name__ == "__main__":
    ...
    upper = numpy.array([map_info.origin.position.x+map_info.resolution*map_info.width, map_info.origin.position.y+map_info.resolution*map_info.height])

    # Settings
    halton_points = 500   # the total amount of points(node) we sample from the configuration space
    disc_radius = 100     # create an edge between two nodes if they are within disc_radius apart
    car_width = 0.33      # consider a car as a square instead of a point in the configuration space
    car_length = 0.33
    collision_delta = 0.05 # discretize an edge using this param -- more description on ObstacleManager
